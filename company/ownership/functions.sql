...
$$
BEGIN
	UPDATE ownership.my_captable_columns SET columnheaders = columnheaders || hstore(index, header);
END;
$$
LANGUAGE plpgsql;
...
BEGIN
	UPDATE ownership.captable_columns SET columnheaders = columnheaders || NEW.columnheaders;
  	RETURN NEW;
END $$;
