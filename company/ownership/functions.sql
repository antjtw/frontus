...
BEGIN
  ...
  template = replace(template, '{{inviter}}', fullname);
	perform mail.send_mail(NEW.email, concat((SELECT name FROM account.company_table WHERE company = NEW.company), '''s cap table has been shared with you!'), template);
  RETURN NEW;
END $$;
